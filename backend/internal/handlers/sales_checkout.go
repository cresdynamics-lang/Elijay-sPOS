...
func (h *Handler) CheckoutSale(c *gin.Context) {
	...
	for _, item := range req.Items {
		...
		if err := tx.QueryRow(ctx, `
			SELECT COALESCE(spp.price, p.base_price) FROM products p
			JOIN product_variants pv ON pv.product_id = p.id
			LEFT JOIN shop_product_prices spp ON spp.product_id = p.id AND spp.shop_id = $2
			WHERE pv.id = $1
		`, variantID, sellingShopID).Scan(&listPrice); err != nil {
