...
func (h *Handler) ListProductVariants(c *gin.Context) {
	...
	query := `
		SELECT pv.id::text, p.name, COALESCE(parent.name, cat.name), pv.sku, pv.size, pv.color, `
	if shopID != "" {
		query += `COALESCE(spp.price, p.base_price)`
	} else {
		query += `p.base_price`
	}
	if shopID != "" {
	...
	if shopID != "" {
		...
		query += fmt.Sprintf(` LEFT JOIN shop_product_prices spp ON spp.product_id = p.id AND spp.shop_id = $%d`, n)
