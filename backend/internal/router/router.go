...
import (
	...
	"github.com/cresdynamics-lang/pos-elijays/backend/internal/handlers"
	"github.com/cresdynamics-lang/pos-elijays/backend/internal/middleware"
...
func New(pool *pgxpool.Pool, cfg config.Config) *gin.Engine {
	...
	variantRead := middleware.RequireAnyPermission(auth.PermInventory, auth.PermSales, auth.PermSalesC)
			...
			protected.GET("/shop-prices", middleware.RequirePermission(auth.PermInventory), h.ListShopProductPrices)
			protected.POST("/shop-prices", middleware.RequirePermission(auth.PermInventoryE), h.SetShopProductPrice)
