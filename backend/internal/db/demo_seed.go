...
import (
	...
	"github.com/cresdynamics-lang/pos-elijays/backend/internal/auth"
	"github.com/cresdynamics-lang/pos-elijays/backend/internal/catalog"
	"github.com/cresdynamics-lang/pos-elijays/backend/internal/models"
...
type demoProduct struct {
	...
	Cost         float64
	Colors       []string
	SizeProfile  string // apparel, cap, none
}
...
func EnsureDemoCatalog(ctx context.Context, pool *pgxpool.Pool) {
	...
	products := []demoProduct{
		{"Presidential Shirt", "presidential-shirt", "presidential", "Elijays", 8500, 4200, []string{"White", "Blue"}, "apparel"},
		{"Merino Sweater", "merino-sweater", "sweaters", "Elijays", 12000, 5500, []string{"Navy", "Grey"}, "apparel"},
		{"Knitted Polo", "knitted-polo", "knitted-polos", "Elijays", 6500, 3200, []string{"Black", "White"}, "apparel"},
		{"Silk Tie", "silk-tie", "belts-ties", "Elijays", 3500, 1200, []string{"Burgundy", "Navy"}, "none"},
		{"Elijays Cap", "elijays-cap", "caps-hats", "Elijays", 2500, 900, []string{"Navy", "Black"}, "cap"},
	}
	...
	for rows != nil && rows.Next() {
		...
		if rows.Scan(&id, &name) == nil {
			if name == "Elijays — Shop One" {
				shopIDs["shop_one"] = id
			}
			if name == "Elijays — Shop Two" {
				shopIDs["shop_two"] = id
			}
		}
	}
	...
	urows, _ := pool.Query(ctx, `SELECT id, email FROM users WHERE email LIKE '%@elijays.co.ke'`)
	...
	for _, dp := range products {
		...
		for shopKey, shopID := range shopIDs {
			shopPrice := dp.Price
			if shopKey == "shop_two" {
				shopPrice = float64(int(dp.Price*1.1 + 0.5)) // Shop Two ~10% higher
			}
			pool.Exec(ctx, `
	...
	recordDemoSale(ctx, pool, shopIDs["shop_two"], cashierIDs["cashier2@elijays.co.ke"], "presidential-shirt-L-White", 2, 7500, "cash")
	recordDemoSale(ctx, pool, shopIDs["shop_one"], cashierIDs["admin@elijays.co.ke"], "merino-sweater-L-Navy", 1, 12000, "mpesa")
	recordDemoSale(ctx, pool, shopIDs["shop_two"], cashierIDs["cashier2@elijays.co.ke"], "silk-tie-Burgundy", 1, 3500, "cash")
...
func ensureDemoStaff(ctx context.Context, pool *pgxpool.Pool) {
	...
	staffList := []staff{
		{"James Kariuki", "cashier1@elijays.co.ke", "cashier", "Elijays — Shop One"},
		{"Mary Wanjiku", "cashier2@elijays.co.ke", "cashier", "Elijays — Shop Two"},
	}
